...
- source rev: `ae896c0` — "feat(deps): drop librustzcash/orchard forks for
  upstream crates.io releases". Clean tree (only untracked `pkg-parallel/`).
...
- sha256(zafu_wasm_bg.wasm) = 106f9376b0d8ac1a1050d0f374e945682eb04be49ce2a9f8068ca1c719d23d3c
...
- all three blobs byte-identical: sha256 `106f9376…`
...
- `zafu_wasm.js` and `zafu_wasm.d.ts` regenerated BYTE-IDENTICAL to the
  ...
  so no TypeScript changes were required.
...
NOT verified: no transaction was broadcast from this blob. An on-chain
round-trip (turnstile first — it has a known-good mined reference at block
3,436,797 — then z→t) is still required before release.
