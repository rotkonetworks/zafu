/**
 * Editable Injective RPC endpoint pool, shown as a subnetwork under Penumbra in
 * network settings. Burner-address routing to Injective (CCTP + native USDC.inj
 * destination) rotates across this list per address so no single provider can
 * link them. Persisted via useInjectiveRpcPool.
 */

import { useEffect, useState } from 'react';
import { Button } from '@repo/ui/components/ui/button';
import { cn } from '@repo/ui/lib/utils';
import { useInjectiveRpcPool, defaultInjectiveRpcPool } from '../../../hooks/injective-rpc';

export const InjectiveEndpointsEditor = () => {
  const { pool, isCustom, save, reset } = useInjectiveRpcPool();
    ...
    <div className='flex flex-col gap-2'>
      <div className='flex items-center justify-between'>
        <span
          ...
          title='burner-address routing to injective (cctp + native usdc.inj) rotates across these endpoints, so no single provider can link all of your addresses. add your own for more separation.'
        >
      ...
      {draft.filter(s => s.trim()).length === 0 && (
        <p className='text-label text-fg-dim lowercase'>
          empty - saving reverts to the {defaultInjectiveRpcPool().length} shipped defaults.
        </p>
      )}
    </div>
  );
