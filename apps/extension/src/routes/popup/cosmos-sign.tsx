  ...
  const msgs = signDoc.msgs.map(msg => {
    ...
    if (msg.type === 'cosmos-sdk/MsgTransfer') {
      ...
      return {
        type: 'IBC Transfer',
        to: v.receiver,
        amount: `${amt} ${symbol || v.token.denom}`,
        ...
        <div className='grow overflow-auto p-4 flex flex-col gap-4'>
          ...
          {txSummary && (
            <div className='rounded-md border border-border-soft bg-elev-1 p-3'>
              ...
              {txSummary.msgs.map(
                (
                  msg: { type: string; to?: string; amount?: string; channel?: string },
                  i: number,
                ) => (
                  <div key={i}>
  ...
  if (step === 'broadcasting') {
    return (
      <div className='flex h-screen flex-col items-center justify-center bg-canvas gap-4'>
        ...
        <p className='text-[13px] text-fg lowercase tracking-[0.02em]'>
    ...
    return (
      <div className='flex h-screen flex-col items-center justify-center bg-canvas gap-4 p-6'>
        ...
        <div className='flex flex-col items-center gap-1'>
          ...
          <h2 className='text-[18px] text-fg-high lowercase tracking-[-0.01em]'>
            transaction sent
          </h2>
        </div>
        {txHash && (
          <p className='text-[10px] text-fg-muted tabular break-all text-center'>{txHash}</p>
        )}
