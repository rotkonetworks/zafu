  ...
  switch (type) {
    ...
    case 'zafu_dkg_join': {
      void (async () => {
        ...
        const rawPrefix = String(msg['labelPrefix'] || new URL(gate.origin).host || 'multisig');
    ...
    case 'zafu_delete_multisig': {
      // Destructive op. Three gates, all collapsed to "denied" on the wire so
      // a probing site can't distinguish which check tripped:
      //   1. caller has 'frost' capability
      //   2. multisigLabel non-empty
      //   3. a vault matches AND was created by this same origin
      //      (createdByOrigin recorded on DKG-join; older vaults predate the
      //      field and are intentionally unaddressable via this API)
      void (async () => {
        const gate = await requireCapability(sender, 'frost', sendResponse);
        if (!gate) return;
