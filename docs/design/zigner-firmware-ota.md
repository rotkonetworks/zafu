...
- Wallet-side UX is one trustworthy decision: *"signed & verified — upgrade? Y/N"*,
...
### 2.2 Attackers
- **A1** malicious/compromised firmware *server* (tampered manifest/image; serves
  a *valid older* image to downgrade).
...
- **Device is the sole root of trust for firmware.** Holds the pinned Zafu update
  public key; verifies *everything* it runs against it; never trusts wallet/server
  for authenticity.
...
There is exactly **one** definition of each. Remove any other wording. The image_sig is
over the signed *image header* field set above (NOT over manifest field numbers 1..4) —
...
- **Rotation:** a *new* anchor may be installed only by a firmware image signed by
  the *current* anchor (the existing signed root is the only thing allowed to
...
- **Activation:** only on an explicit human tap; then reboot; the new slot is NOT
  considered durable until its *own* firmware, after boot, confirms
...
- **Image replay is not an attack:** a replayed *signed* image is still a valid signed
...
- **Signed revocation of compromised old versions (supply-chain half):** the Zafu signer
  can *expire* a known-critical-vuln `(version, key_id)` by signing a small revocation
...
- **Proportionate scope:** the only downgrade that could matter is a *physical* attacker
