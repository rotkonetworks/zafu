...
the session key is safe if _either_ component holds. This is one
...
classical is a _real-time_ forge risk only — it does NOT
...
_non-standardized_ problem (see step 3).
...
is a _deliberate hybrid-KEM construction_ (PQ-Noise / combiner
...
- `packages/zid` identity signatures (ed25519) — the classical
  _auth_ gap. Lower urgency (real-time only). PQ sigs (ML-DSA /
...
- [ ] 6. Out of scope phase 1, tracked separately: PQ identity
     _signatures_ (non-standardized; revisit only if real-time
...
- No claim that the existing zid Noise impl is x25519 _only_ until
...
working precedent for the _confidentiality_ half; the
_authenticity_ half (falconed) was deliberately deferred there too.
